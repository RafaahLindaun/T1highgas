...
Set-Content -LiteralPath (Join-Path $Base 'helper.token') -Value $token -Encoding ASCII -NoNewline

$helper = Join-Path $Base 'highgas-helper.exe'
$tokenFile = Join-Path $Base 'helper.token'
$action = New-ScheduledTaskAction -Execute $helper -Argument ('--token-file "{0}"' -f $tokenFile) -WorkingDirectory $Base
...
for ($i=0;$i -lt 80;$i++) {
  ...
  try {
    ...
    if ($h.helper -and $h.version -eq '2.0.0' -and $h.platform -eq 'windows' -and $h.engineReady) { $ok=$true; break }
  } catch {}
}
