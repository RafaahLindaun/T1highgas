...
function Down {
  $s = Read-State
  # Fail-closed: primeiro retira a rota de apps; o bloqueio continua ativo.
  ...
  Get-Process tor -ErrorAction SilentlyContinue | Where-Object { $_.Path -like "$Base*" } | Stop-Process -Force -ErrorAction SilentlyContinue
  if ($s -and $s.PSObject.Properties.Name -contains 'tunAlias' -and $s.tunAlias) {
    try { Set-DnsClientServerAddress -InterfaceAlias ([string]$s.tunAlias) -ResetServerAddresses -ErrorAction SilentlyContinue } catch {}
  }
  Remove-HighGASRules
  Restore-Firewall $s
...
function Tor-Info([string]$Country) {
  ...
  try {
    ...
    if ([string]$j.country -ne $Country) { return $null }
    return $j
  } catch { return $null }
}
...
function Start-Tor([object]$Physical,[string]$Country,[string]$ExitCode) {
  ...
  for ($i=0;$i -lt 360;$i++) {
    ...
    if ((Test-Path $TorLog) -and (Select-String -LiteralPath $TorLog -SimpleMatch 'Bootstrapped 100%' -Quiet -ErrorAction SilentlyContinue)) { break }
    Start-Sleep -Milliseconds 500
  }
  if (!(Test-Path $TorLog) -or !(Select-String -LiteralPath $TorLog -SimpleMatch 'Bootstrapped 100%' -Quiet -ErrorAction SilentlyContinue)) { throw 'O Tor não concluiu o bootstrap.' }
  $probe = $null
  for ($i=0;$i -lt 12;$i++) {
...
function Pin-TorGuards([int]$TorPid,[object]$Physical) {
...
function Enable-KillSwitch([object]$Physical,[object]$FirewallState) {
  Remove-HighGASRules
  ...
  # Mantém renovação DHCP sem abrir tráfego de aplicações.
...
function Up([string]$Code) {
  ...
  try {
    ...
    if ($v[-1] -ne 1 -and $v -notcontains 'VERIFIED=1') { throw 'A verificação completa não passou; o HighGAS desligou para evitar vazamento.' }
...
function Status {
  ...
  if (!(Get-Process -Id ([int]$s.torPid) -ErrorAction SilentlyContinue) -or !(Get-Process -Id ([int]$s.tunPid) -ErrorAction SilentlyContinue)) { 'disconnected'; return }
...
switch ($Action) {
  ...
  'status' { Status }
  'verify' { [void](Verification-Lines) }
}
