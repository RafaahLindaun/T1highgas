...
$TunnelPrefixes = @('0.0.0.0/1','128.0.0.0/1')
...
function Restore-Firewall([object]$State) {
  ...
  foreach ($f in @($State.firewall)) {
    try {
      Set-NetFirewallProfile -Profile ([string]$f.Name) -DefaultOutboundAction ([string]$f.DefaultOutboundAction) | Out-Null
    } catch {
      Set-NetFirewallProfile -Profile ([string]$f.Name) -DefaultOutboundAction Allow -ErrorAction SilentlyContinue | Out-Null
    }
  }
}
...
function Remove-HighGASRoutes([object]$State) {
  ...
  if ($State.PSObject.Properties.Name -contains 'tunAlias' -and $State.tunAlias) {
    foreach ($prefix in $TunnelPrefixes) {
      Get-NetRoute -InterfaceAlias ([string]$State.tunAlias) -DestinationPrefix $prefix -PolicyStore ActiveStore -ErrorAction SilentlyContinue |
        Remove-NetRoute -Confirm:$false -ErrorAction SilentlyContinue
    }
  }
  if ($State.PSObject.Properties.Name -contains 'guardRoutes' -and $State.guardRoutes) {
    foreach ($prefix in @($State.guardRoutes)) {
      Get-NetRoute -DestinationPrefix ([string]$prefix) -PolicyStore ActiveStore -ErrorAction SilentlyContinue |
...
function Down {
  $s = Read-State
  # Fail-closed: primeiro remove as rotas dos apps; o firewall continua bloqueando saída direta.
...
function Start-Tor([object]$Physical,[string]$Country,[string]$ExitCode) {
  ...
  Remove-Item -LiteralPath (Join-Path $Runtime 'tor-error.log') -Force -ErrorAction SilentlyContinue
  $argLine = "--SocksPort `"127.0.0.1:$SocksPort`" --DNSPort `"127.0.0.1:$DnsPort`" --DataDirectory `"$TorData`" --GeoIPFile `"$GeoIP`" --GeoIPv6File `"$GeoIP6`" --ExitNodes `"{$ExitCode}`" --StrictNodes 1 --ClientUseIPv6 0 --OutboundBindAddress `"$($Physical.IP)`" --Log `"notice stdout`""
  $p = Start-Process -FilePath $TorExe -ArgumentList $argLine -WindowStyle Hidden -PassThru -RedirectStandardOutput $TorLog -RedirectStandardError (Join-Path $Runtime 'tor-error.log')
  for ($i=0;$i -lt 360;$i++) {
    if ($p.HasExited) {
      $msg = Get-Content -LiteralPath $TorLog,(Join-Path $Runtime 'tor-error.log') -Raw -ErrorAction SilentlyContinue
      throw "O Tor encerrou durante a inicialização. $msg"
    }
...
function Enable-KillSwitch([object]$Physical) {
  ...
  New-NetFirewallRule -DisplayName 'HighGAS Tor TCP IPv4' -Group $FirewallGroup -Direction Outbound -Action Allow -Profile Any -Program $TorExe -Protocol TCP -RemoteAddress '0.0.0.0/0' -InterfaceAlias ([string]$Physical.Alias) | Out-Null
...
function Start-Tun {
  Remove-Item -LiteralPath $TunLog,(Join-Path $Runtime 'tun2socks-error.log') -Force -ErrorAction SilentlyContinue
  ...
  for ($i=0;$i -lt 100;$i++) {
    if ($p.HasExited) {
      $msg = Get-Content -LiteralPath $TunLog,(Join-Path $Runtime 'tun2socks-error.log') -Raw -ErrorAction SilentlyContinue
      throw "tun2socks encerrou antes de criar o Wintun. $msg"
    }
  ...
  foreach ($prefix in $TunnelPrefixes) {
    New-NetRoute -DestinationPrefix $prefix -InterfaceAlias $adapter.Name -NextHop $TunAddress -RouteMetric 1 -PolicyStore ActiveStore -ErrorAction Stop | Out-Null
  }
...
function Test-KillSwitch([object]$State) {
  try {
    foreach ($prefix in $TunnelPrefixes) {
      $route = Get-NetRoute -DestinationPrefix $prefix -InterfaceAlias ([string]$State.tunAlias) -PolicyStore ActiveStore -ErrorAction Stop | Select-Object -First 1
...
function Verification-Lines {
  ...
  if ($s) {
    ...
    if ($info) {
      $ip = [string]$info.ip
      $country = ([string]$info.country).ToUpperInvariant()
...
function Up([string]$Code) {
  ...
  try {
    ...
    $state.guardRoutes = @($guards)
    Write-State $state
    $tun = Start-Tun
    $state.tunPid = $tun.Process.Id
    $state.tunAlias = [string]$tun.Adapter.Name
...
function Status {
  ...
  try {
    ...
    if (@($routes).Count -ne @($s.guardRoutes).Count) {
      $s.guardRoutes=@($routes)
      Write-State $s
    }
  } catch {}
